...
You can create a crosswalk across geographic delineations by providing three shapefiles to the `create_cw()` function:
...
First, we can download the 2020 CT county definition (the source) and the 2023 CT county definitions (the target):

```{r dwnld, warning=FALSE, message=FALSE}
library(geolinkr)
library(sf)
library(tigris)
...
From 2020 to 2023, Connecticut's county count increased from 8 to 9 and the delineations changed:

```{r ct_cnty_chg, fig.height=2}
par(mfrow = c(1, 2))
